 <?php
	...
	function reservation(){
		...
		$rname_id = explode('|', $rname);
		// $total = "SELECT status from rooms where status = 0 and rName = '$rname_id[0]'";
	 ...
	 //    }else{
		...
		// if(strcmp($rtype, "Superior Room")==0){
			// $superior = 'SELECT rType from rooms where rType="$rtype" and status=0';
		 ...
		 //    }
		    ...
		    // if($supcount>0){
				// $checksup = "SELECT rName from roombook where  rName = '$rname_id[0]' and (cin >= '$cin' and cin > '$cout') or (cout >= '$cin' and cout < '$cout')";
				$checksup = "SELECT rName from roombook where (cin between '$cin' and '$cout') and (cout between '$cin' and '$cout') and rName = '$rname_id[0]'";
			    ...
			    // echo $checksuperior;die;
			    if(!$checksuperior){

					$query = "INSERT INTO roombook(fullName, phone, rName, rType, cin, cout, room_no) values ('{$fname}', '{$phone}', '{$rname_id[0]}', '{$rtype}', '{$cin}', '{$cout}', '{$rname_id[1]}')";
					$result = mysqli_query($connection, $query);
					if($result) {
			    	...
			    	redirect_to("available_room.php?cin=$cin&cout=$cout");
			    	// echo "<script type='text/javascript'> alert('Not Available!')</script>";
			    }
			// }else{
			// 	echo "<script type='text/javascript'> alert('All $rtype is fully booked. Sorry.')</script>";
			// }
		 // }
		// }
